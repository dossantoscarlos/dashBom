    ...
    <ModuleBlock title="Inteligência e Relatórios" icon="📊">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setCategoryFilter("all")} className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${categoryFilter === "all" ? "bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900" : "bg-zinc-100 text-zinc-600 dark:bg-zinc-800"}`}>Todos</button>
          {REPORT_CATEGORIES.map((cat) => (
            <button key={cat} type="button" onClick={() => setCategoryFilter(cat)} className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${categoryFilter === cat ? "bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900" : "bg-zinc-100 text-zinc-600 dark:bg-zinc-800"}`}>{cat}</button>
          ))}
        </div>
        {Array.from(grouped.entries()).map(([category, templates]) => (
          <section key={category}>
            <h2 className="mb-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400">{category}</h2>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {templates.map((report) => (
                <button key={report.id} type="button" onClick={() => { setSelectedId(report.id); setResult(null); }}
                  className={`rounded-xl border p-4 text-left transition ${selectedId === report.id ? "border-zinc-900 bg-zinc-50 dark:border-zinc-100 dark:bg-zinc-900" : "border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950"}`}>
                  <span className="text-xl" aria-hidden>{report.icon}</span>
                  <p className="mt-2 text-sm font-bold">{report.title}</p>
                  <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400 line-clamp-2">{report.description}</p>
                </button>
              ))}
            </div>
          </section>
        ...
        {selected && (
          <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{selected.description}</p>
            <div className="mt-3 rounded bg-zinc-100 p-2 font-mono text-[10px] dark:bg-zinc-900">{selected.query}</div>
            ...
            {result && !loading && (
              <div className="mt-6">
                <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-zinc-400">Pré-visualização — {periodo}</h4>
                <DataTable
