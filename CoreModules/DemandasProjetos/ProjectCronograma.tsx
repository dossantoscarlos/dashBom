...
export function ProjectCronograma({
  ...
  return (
    <div className="flex flex-col gap-5 font-sans text-xs antialiased select-none">
      {/* ── BARRA SUPERIOR DE AÇÕES E IDENTIFICAÇÃO DO GANTT ── */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[#E2E8F0] dark:border-zinc-800 pb-4">
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={() => onNavigateTab("visao_geral")}
            className="px-3.5 py-2 rounded-xl bg-white dark:bg-zinc-900 border border-[#E2E8F0] dark:border-zinc-800 hover:bg-[#F8FAFC] dark:hover:bg-zinc-800 text-[#10213D] dark:text-zinc-100 font-bold text-xs transition shadow-2xs cursor-pointer"
          >
            Detalhes do projeto
          </button>
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-[#EFF6FF] dark:bg-blue-950/40 border border-[#BFDBFE] dark:border-blue-900 hover:bg-[#DBEAFE] text-[#1D4ED8] dark:text-blue-300 font-extrabold text-xs transition shadow-2xs flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            title="Sincronizar prazos e marcos com a Agenda da Campanha"
          >
            <CalendarCheck className="h-4 w-4 text-[#2563EB] dark:text-blue-400" strokeWidth={2} />
            <span>{isSyncingAgenda ? "Sincronizando..." : "Sincronizar com Agenda"}</span>
          </button>

          <button
            type="button"
            onClick={handleExportGantt}
            className="px-3.5 py-2 rounded-xl bg-white dark:bg-zinc-900 border border-[#E2E8F0] dark:border-zinc-800 hover:bg-[#F8FAFC] dark:hover:bg-zinc-800 text-[#10213D] dark:text-zinc-100 font-bold text-xs transition shadow-2xs flex items-center gap-1.5 cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2">
          <span className="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#F8FAFC] dark:bg-zinc-900 border border-[#E2E8F0] dark:border-zinc-800 rounded-xl text-[11px] font-bold text-[#1D4ED8] dark:text-blue-400">
            <span className="h-2 w-2 rounded-full bg-[#3B82F6] animate-pulse" />
            Agenda Sincronizada ({lastAgendaSync})
          </span>
          ...
          >
            ...
            <span>Novo marco (Milestone)</span>
          </button>
        </div>
      </div>

      {/* ── BARRA DE INDICADORES DE CRONOGRAMA GANTT ── */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-[#E2E8F0] dark:border-zinc-800 shadow-2xs">
        <div className="flex items-center gap-3">
          <CalendarIcon className="h-7 w-7 text-[#1264F3] bg-[#EAF2FF] dark:bg-blue-950/60 p-1.5 rounded-xl" />
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-[#64748B]">Início do Gantt</span>
            <span className="text-sm font-black text-[#10213D] dark:text-zinc-100">02/06/2025</span>
          </div>
        </div>

        <div className="flex items-center gap-3 border-l border-[#F1F5F9] dark:border-zinc-800 pl-3">
          <CalendarIcon className="h-7 w-7 text-[#008B63] bg-[#E8F7F1] dark:bg-emerald-950/60 p-1.5 rounded-xl" />
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-[#64748B]">Término previsto</span>
            <span className="text-sm font-black text-[#10213D] dark:text-zinc-100">07/09/2025</span>
          </div>
        </div>

        <div className="flex items-center gap-3 border-l border-[#F1F5F9] dark:border-zinc-800 pl-3">
          <Clock className="h-7 w-7 text-[#7928F5] bg-[#F3EAFF] dark:bg-purple-950/60 p-1.5 rounded-xl" />
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-[#64748B]">Duração total</span>
            <span className="text-sm font-black text-[#10213D] dark:text-zinc-100">98 dias</span>
          </div>
        </div>

        <div className="flex items-center gap-3 border-l border-[#F1F5F9] dark:border-zinc-800 pl-3">
          <CheckCircle2 className="h-7 w-7 text-[#008B63] bg-[#E8F7F1] dark:bg-emerald-950/60 p-1.5 rounded-xl" />
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-[#64748B]">Progresso global</span>
            <span className="text-sm font-black text-[#008B63]">65%</span>
          </div>
        </div>

        <div className="flex items-center gap-3 border-l border-[#F1F5F9] dark:border-zinc-800 pl-3">
          <CalendarCheck className="h-7 w-7 text-[#2563EB] bg-[#EFF6FF] dark:bg-blue-950/60 p-1.5 rounded-xl" />
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-[#64748B]">Eventos Campanha</span>
            <span className="text-sm font-black text-[#1D4ED8] dark:text-blue-400">6 agendados</span>
          </div>
        </div>
      </div>

      {/* ── BARRA DE FERRAMENTAS DO CRONOGRAMA GANTT ── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-zinc-900 p-3 rounded-2xl border border-[#E2E8F0] dark:border-zinc-800 shadow-2xs">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            placeholder="Buscar tarefa ou entrega no Gantt..."
            className="h-9 w-full pl-9 pr-3 rounded-xl border border-[#E2E8F0] dark:border-zinc-800 bg-[#F8FAFC] dark:bg-zinc-800 text-xs text-[#10213D] dark:text-zinc-100 focus:bg-white focus:border-[#1264F3] focus:outline-hidden"
          />
        </div>

        <div className="flex items-center gap-2">
          {/* Seletor de Zoom (Dia, Semana, Mês) */}
          <div className="flex items-center bg-[#F8FAFC] dark:bg-zinc-800 border border-[#E2E8F0] dark:border-zinc-700 p-0.5 rounded-xl">
            <button
              type="button"
              onClick={() => setZoomLevel("dia")}
              className={`px-3 py-1 rounded-lg text-[11px] font-extrabold transition cursor-pointer ${
                zoomLevel === "dia"
                  ? "bg-white dark:bg-zinc-900 text-[#10213D] dark:text-zinc-100 shadow-2xs"
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-[11px] font-extrabold transition cursor-pointer ${
                zoomLevel === "semana" ? "bg-[#008B63] text-white shadow-xs" : "text-[#64748B] hover:text-[#10213D]"
              }`}
            >
              Semana
            </button>
          ...
          <span className="font-mono text-xs font-bold text-[#10213D] dark:text-zinc-100 px-3 py-1.5 bg-[#F8FAFC] dark:bg-zinc-800 rounded-xl border border-[#E2E8F0] dark:border-zinc-700">
            02/06 — 07/09/2025
          </span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-[#E2E8F0] dark:border-zinc-800 shadow-2xs overflow-hidden flex flex-col">
        <div className="overflow-x-auto no-scrollbar">
          <div className="min-w-[1100px] flex flex-col">
            {/* Cabeçalho da Tabela e Timeline do Gantt */}
            <div className="grid grid-cols-[440px_1fr] bg-[#F8FAFC] dark:bg-zinc-800/80 border-b border-[#E2E8F0] dark:border-zinc-800 text-xs font-bold text-[#64748B]">
              <div className="grid grid-cols-[180px_80px_60px_50px_70px] p-3 border-r border-[#E2E8F0] dark:border-zinc-800 items-center">
                <span>Estrutura / Item</span>
                <span>Responsável</span>
                <span>Início/Fim</span>
                <span>Prog.</span>
                <span className="text-center">Campanha</span>
              </div>

              {/* Cabeçalho das Semanas e Meses */}
              <div className="flex flex-col">
                <div className="grid grid-cols-4 text-center border-b border-[#E2E8F0] dark:border-zinc-800 py-1 font-extrabold text-[#10213D] dark:text-zinc-100">
                  <span>junho 2025</span>
                  <span>julho 2025</span>
                  <span>agosto 2025</span>
                  <span>setembro 2025</span>
                </div>
                <div className="grid grid-cols-16 text-[10px] text-center text-[#64748B] py-1 font-mono border-b border-[#E2E8F0] dark:border-zinc-800">
                  <span>02 - 08</span>
                  <span>09 - 15</span>
                  <span>16 - 22</span>
                  <span>23 - 29</span>
            ...
            {/* Corpo Hierárquico das Fases do Projeto no Gantt */}
            <div className="flex flex-col relative divide-y divide-[#F1F5F9] dark:divide-zinc-800">
              {/* Linha Vertical 'Hoje' na Linha do Tempo */}
              <div
                className="absolute top-0 bottom-0 w-0.5 bg-[#1264F3] z-20 pointer-events-none flex flex-col items-center"
                style={{ left: "calc(440px + 18%)" }}
              >
              ...
              {cronogramaData.map((fase) => {
                ...
                return (
                  <React.Fragment key={fase.id}>
                    {/* Linha da Fase Pai (Barra Agrupadora de Gantt) */}
                    <div className="grid grid-cols-[440px_1fr] items-center hover:bg-[#F8FAFC] dark:hover:bg-zinc-800/40 transition text-xs font-extrabold text-[#10213D] dark:text-zinc-100 py-2.5">
                      <div className="grid grid-cols-[180px_80px_60px_50px_70px] px-3 items-center border-r border-[#E2E8F0] dark:border-zinc-800">
                        <button
                          type="button"
                      ...
                      {/* Barra Visual de Fase Pai na Timeline */}
                      <div className="px-4 relative flex items-center h-full">
                        <div
                          className="h-3.5 rounded-full bg-[#008B63] dark:bg-emerald-600 shadow-2xs relative overflow-hidden flex items-center justify-end pr-2"
                          style={{
                        ...
                        >
                          <span className="text-[8px] font-black text-white/90">{fase.progress}%</span>
                        </div>
                      </div>
                    </div>

                    {/* Itens Filhos da Fase (Barras e Marcos Individuais do Gantt) */}
                    {isExpanded &&
                      fase.children?.map((item, idx) => (
                        <div
                          key={item.id}
                          className="grid grid-cols-[440px_1fr] items-center hover:bg-[#F8FAFC] dark:hover:bg-zinc-800/40 transition text-xs text-[#10213D] dark:text-zinc-100 py-2 pl-4"
                        >
                          <div className="grid grid-cols-[180px_80px_60px_50px_70px] px-3 items-center border-r border-[#E2E8F0] dark:border-zinc-800">
                            ...
                            <span className="text-[10px] font-bold text-[#10213D] dark:text-zinc-100">{item.progress}%</span>
                            ...
                            <div className="flex justify-center">
                              <button
                                ...
                                className="p-1 rounded-md text-[#2563EB] hover:bg-[#EFF6FF] dark:hover:bg-blue-950/60 transition cursor-pointer"
                          ...
                          {/* Barras e Marcos do Gantt para cada Item */}
                          <div className="px-4 relative flex items-center h-full">
                            {item.isMilestone ? (
                              <div
                              ...
                              >
                                <span className="h-4 w-4 rotate-45 bg-[#008B63] shadow-xs inline-block group-hover:scale-125 transition border border-white dark:border-zinc-900" />
                                <span className="text-[9px] font-extrabold text-[#008B63] dark:text-emerald-400">
                                  {item.code}
                                </span>
                                <span className="text-[8px] px-1 py-0.2 rounded bg-[#EFF6FF] dark:bg-blue-950/80 text-[#1D4ED8] dark:text-blue-300 font-bold border border-[#BFDBFE] dark:border-blue-900">
                                  Agenda
                                </span>
                              </div>
                            ) : (
                              <div
                                className={`h-3 rounded-full shadow-2xs flex items-center justify-end pr-1 text-[8px] font-black text-white ${
                                  item.status === "Concluída"
                                    ? "bg-[#008B63]"
                                    : item.status === "Atrasada"
                                    ? "bg-[#EF4444]"
                                ...
                                title={`${item.name} — ${item.progress}% concluído`}
                              >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 p-3 bg-[#F8FAFC] dark:bg-zinc-800/80 border-t border-[#E2E8F0] dark:border-zinc-800 text-xs font-bold text-[#10213D] dark:text-zinc-100">
          ...
          <span className="text-[10px] text-[#64748B] font-medium">
            Diagrama de Gantt com Sincronização em Tempo Real
          </span>
        </div>
      </div>
      ...
      {showScheduleEventModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-[#E2E8F0] dark:border-zinc-800 shadow-2xl max-w-lg w-full p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-[#E2E8F0] dark:border-zinc-800 pb-3">
              <h3 className="text-base font-extrabold text-[#10213D] dark:text-zinc-100 flex items-center gap-2">
