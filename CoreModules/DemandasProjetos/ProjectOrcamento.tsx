...
export function ProjectOrcamento({
  ...
  const handleCreateExpense = () => {
    ...
    toast("Nova despesa registrada com sucesso! Status: Aguardando aprovação.");
    setDescription("");
    setDocNumber("");
    setSupplier("");
    setValueAmount("");
  ...
  return (
    <div className="flex flex-col gap-6 font-sans text-xs antialiased select-none">
      {/* ── BARRA SUPERIOR DE AÇÕES (CONFORME IMAGEM 3) ── */}
      ...
      {/* ── CARD INDICADORES FINANCEIROS CHAVE (CONFORME IMAGEM 3) ── */}
      ...
      {/* ── GRID DE TABELA DE CATEGORIAS E GRÁFICO DE DISTRIBUIÇÃO (IMAGEM 3) ── */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
        ...
        {/* Gráfico Rosca de Distribuição e Alertas Orçamentários (Imagem 3) */}
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white p-5 rounded-2xl border border-[#E2E8F0] shadow-2xs flex flex-col gap-4">
            ...
            <div className="flex items-center justify-center relative my-2">
              {/* Gráfico Donut SVG Fidelidade Visual */}
            ...
            {/* Legenda de Distribuição com Valores */}
            <ul className="flex flex-col gap-1.5 text-xs">
              <li className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[#00A978]" />
                  <span className="text-[#64748B]">Obras e infraestrutura</span>
                </div>
                <span className="font-extrabold text-[#10213D]">R$ 240.000,00 (50%)</span>
              </li>

              <li className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[#1264F3]" />
                  <span className="text-[#64748B]">Equipamentos</span>
                </div>
                <span className="font-extrabold text-[#10213D]">R$ 100.000,00 (21%)</span>
              </li>

              <li className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[#7928F5]" />
                  <span className="text-[#64748B]">Serviços técnicos</span>
                </div>
                <span className="font-extrabold text-[#10213D]">R$ 70.000,00 (15%)</span>
              </li>

              <li className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[#F59E0B]" />
                  <span className="text-[#64748B]">Comunicação</span>
                </div>
                <span className="font-extrabold text-[#10213D]">R$ 30.000,00 (6%)</span>
              </li>

              <li className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[#00C49F]" />
                  <span className="text-[#64748B]">Reserva de contingência</span>
                </div>
                <span className="font-extrabold text-[#10213D]">R$ 40.000,00 (8%)</span>
              </li>
            </ul>
          </div>
      ...
      {/* ── TABELA DE MOVIMENTAÇÕES FINANCEIRAS (IMAGEM 3) ── */}
      <div className="bg-white p-5 rounded-2xl border border-[#E2E8F0] shadow-2xs flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E2E8F0] pb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => toast("Visualização dos comprovantes anexados aberta!")}
              className="h-8 px-3 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] hover:bg-slate-100 font-bold text-xs flex items-center gap-1.5 cursor-pointer"
            >
              <FileText className="h-3.5 w-3.5 text-[#1264F3]" />
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded text-[10px] font-extrabold uppercase border ${
                        tx.status === "Pago"
                          ...
                          : tx.status === "Aguardando aprovação"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-2xl max-w-md w-full p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-3">
              <h3 className="text-base font-extrabold text-[#10213D]">Registrar Nova Despesa</h3>
